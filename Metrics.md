# HackrNews Metrics

## Code Distribution 

![code_distribution](./swift-code-metrics/code_distribution.pdf)

## Code Distribution (Hackr News App)

![](./swift-code-metrics/code_distribution_hackrnewsapp__hackrnewsapp__hackrnewsapp.pdf)

## Code Distribution (Hackr News Framework)

![code-distribution](./swift-code-metrics/code_distribution_hackrnews__hackrnews.pdf)

## Code Distribution (iOS Hackr News Framework)

![code-distribution](./swift-code-metrics/code_distribution_hackrnewsios__hackrnewsios.pdf)



## Dependencies Graph

![dependencies](./swift-code-metrics/dependencies_graph.pdf)

## Lines of Code (LOC)

![loc](./swift-code-metrics/lines_of_code_-_loc.pdf)

## External dependencies Graph

![external_dependencies_graph](./swift-code-metrics/external_dependencies_graph.pdf)